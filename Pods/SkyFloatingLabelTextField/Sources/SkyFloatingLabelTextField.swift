...
open class SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    ...
    dynamic open var titleFont: UIFont? {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    fileprivate func updateTitle() {
        if let placeholder = title, let font = titleFont ?? font {
            attributedPlaceholder = NSAttributedString(
                string: placeholder,
                attributes: [NSForegroundColorAttributeName: titleColor, NSFontAttributeName: font]
            )
        }
    }
